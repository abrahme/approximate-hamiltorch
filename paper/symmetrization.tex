...
volume preserving. We show that this is \emph{not} sufficient for them to
serve as a valid Hamiltonian Monte Carlo proposal, and give a construction
that repairs the deficiency exactly, at no cost in parameters.
...
method; here the adjoint is realised as $R \circ \Phi^{-1} \circ R$. Enforcing
exact invertibility so that a learned proposal admits a tractable reverse move
is likewise established in the neural-MCMC literature \cite{song2017nice,
levy2018generalizing}. Our contribution is the observation that SNN surrogates
for HMC require it --- they supply \eqref{eq:vol} but violate \eqref{eq:inv}
--- together with the closed-form layer inverses that make \eqref{eq:psi}
exact for the LA- and G-SympNet architectures.
