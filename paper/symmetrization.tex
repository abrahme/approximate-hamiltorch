...
volume-preserving involutions. What we add is the observation that SNN
surrogates for HMC require this structure --- they supply \eqref{eq:vol} but
violate \eqref{eq:inv}, so the resulting chain is not asymptotically exact ---
together with closed-form layer inverses that make \eqref{eq:psi} exact for the
LA- and G-SympNet architectures.
