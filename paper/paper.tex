...
\caption{Samples generated from each surrogate method (a) and (b) the
reversibility of the Hamiltonian paths for the multivariate Gaussian. The paths
generated by the unsymmetrized SNNs are not reversible, while the
integrator-based methods are. The cause is \emph{not} that the SNNs are only
approximately symplectic --- they are exactly symplectic, by construction.
Rather, as \Cref{sec:symmetrization} shows, symplecticity is only one of the two
conditions a Metropolis proposal requires, and the architecture supplies nothing
that enforces the other. The symmetrized construction of \eqref{eq:psi} repairs
this exactly.}\label{figure:gaussian}
\end{figure}
